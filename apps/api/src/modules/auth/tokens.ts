...
/**
 * Cryptographically random single-use token, base64url-encoded. Shared by
 * every token-based flow in this app (auth tokens here, invitation tokens
 * in modules/invitations) so the entropy/encoding can't drift between them.
 */
 ...
 *
 * Also stamps `passwordChangedAt` - getSessionUser (src/utils/auth.ts)
 * checks this against a session's JWT `iat` claim to reject any session
 * issued before this reset, so a stolen session cookie doesn't survive its
 * own victim's reset indefinitely.
 */
